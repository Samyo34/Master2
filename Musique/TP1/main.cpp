...
FMOD_RESULT F_CALLBACK cb_question1 (FMOD_SOUND *sound, void *data, unsigned int datalen)
{
	...
	// un échantillon à gauche puis à droite (format "signed short" = entier entre -32768 et 32767)
        ...
        *stereo16bitbuffer++ = (signed short) (ampGauche * sin(2*PI*freq*t+distorsion*sin(t))*32767.0f*adsr(t));
        *stereo16bitbuffer++ = (signed short) (ampDroite * sin(2*PI*freq*t+distorsion*sin(t))*32767.0f*adsr(t));
	    t += 1.0f/44100.0f ;
    }
...
float Attack=0.1, Delay=0.1, Duree=2, Sustain=0.8, Release=1.5 ;
...
float adsr (float x)
{
    ...
    if(x<Attack){
        return x/Attack;
    }else if(x<Attack+Delay){
        return ((1-Sustain)*(Attack+Delay-x)/Delay + Sustain);
    }else if(x < Attack+Delay+Duree){
        return Sustain;
    }else if(x < Attack+Delay+Duree+Release){
        return (Sustain*(Attack+Delay+Duree+Release-x)/Release);
    }
    return 0 ;
}
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        if (kbhit())
        {
            ...
            case '*' :
            {
                ampDroite = ampDroite * 2.0;
                ampGauche = ampGauche * 2.0;
                break;
            }
            case '/' :
            {
